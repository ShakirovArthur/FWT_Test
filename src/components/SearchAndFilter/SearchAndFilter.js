...
export const SearchAndFilter = ({updateText, updateFromDate, updateBeforeDate, filterAuthor, filterLocation,fromDate,beforeDate,theme}) => {
    ...
    return (
        <div className='search-filter'>
            <Input isDarkTheme={theme === 'dark'} placeholder='Name' className='input-search' onChange={handleChange}/>
                <Select displayEmpty className={theme === 'dark' ? 'select-filter dark' : 'select-filter'} onChange={handleChangeFilterAuthor}  defaultValue={''}>
                    <MenuItem value='' selected>Author</MenuItem>
                    {authors?.map((author) => (
                        <MenuItem key={author?.id} value={author?.id} title={author?.name}>{author?.name.length > 15 ? `${author?.name.slice(0,15)}...` : author?.name}</MenuItem>
                    ))}
                </Select>
                <Select displayEmpty className={theme === 'dark' ? 'select-filter dark' : 'select-filter'} onChange={handleChangeFilterLocation} defaultValue={''}  MenuProps={{
                    PaperProps: {
                        style: {
                            maxHeight: 376,
                            overflowY: 'auto'
                    ...
                    {locations?.map((site) => (
                        <MenuItem  key={site?.id} value={site?.id} title={site?.location}>{site?.location.length > 15 ? `${site?.location.slice(0,15)}...` : site?.location}</MenuItem>
                    ))}
                </Select>
            <Select className={theme === 'dark' ? 'select-filter dark' : 'select-filter'} open={isOpen} onOpen={handleOpen} onClose={handleClose}
